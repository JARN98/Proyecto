...
public interface HabitacionRepository extends JpaRepository<Habitacion, Long> {
	...
	@Query(nativeQuery=true, value="SELECT * FROM HABITACION LEFT JOIN RESERVA ON (HABITACION.ID = RESERVA.HABITACION_ID) WHERE (?1 NOT BETWEEN FECHAINICIO AND FECHAFIN AND ?2 NOT BETWEEN FECHAINICIO AND FECHAFIN AND NOT(?1  <= FECHAINICIO AND ?2  >= FECHAFIN) AND TIPOHAB LIKE '?3') OR FECHAINICIO IS NULL")
